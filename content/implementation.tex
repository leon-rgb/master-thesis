...
For this section we want to explain how the request are built and sent. 
For the Request only a few of the last messages are considered to ensure low latency and 


\section{Response Handling and Action Triggering}
As previously described in \cref{subsec:responsehandler}, the Response Handler module interprets server responses, determining necessary client actions such as response analysis and orchestration. This section explains how we implemented this functionality in more detail.
% the response is retrieved as a stream of tokens by the Ollama software thorugh the command ""
% splitting the response string in natural language response and json (if json exists)
