...
As previously described in \cref{subsec:apiclient}, the Request Builder within the API Client \& Request Builder module constructs properly formatted API requests, sends them, parses server responses, and handles asynchronous operations. This section elaborates on the process of building and sending these requests.
...
As previously described in \cref{subsec:responsehandler}, the Response Handler module interprets server responses, determining necessary client actions such as response analysis and orchestration. This section elaborates on the implementation details of this crucial functionality.
...
Upon receiving the complete response, the handler first separates it into two components: the natural language response and, if present, a JSON object. The natural language portion is immediately forwarded to the UI for display, maintaining a fluid conversation flow with the user.
If a JSON object is detected, typically positioned at the end of the response, it undergoes parsing to extract action-related information. The JSON structure follows a predefined format:

\begin{lstlisting}[caption={Action JSON Structure},label=lst:action-json]
{
"action": "intent/action",
"value": "optional value for an action",
"deviceID": "ID of the device",
...
The 'action' field is pivotal, triggering the appropriate function within the client application. The system supports several predefined actions:

\begin{itemize}
\item 'none': No specific action required
\item 'turn-on': Activate a device (applicable to 'socket' type devices)
\item 'turn-off': Deactivate a device (applicable to 'socket' type devices)
\item 'change-temperature': Adjust temperature (requires a 'value' and applicable to 'room-climate-control' devices)
\end{itemize}

Different device types have varying capabilities:

\begin{itemize}
\item Socket devices can be turned on or off.
\item Thermostats provide temperature readings but don't accept direct commands through this interface.
\item Room Climate Control devices manage the setpoint temperature for all thermostats in a room.
\item Door-window contacts provide open/closed status but don't accept actions.
\end{itemize}

The Response Handler utilizes the current device list, always provided in the chat history, to validate and execute actions. This ensures that actions are only triggered for existing devices in their current state.
When an action is identified, the handler invokes the corresponding method in the Smart Home System interface. For instance, a 'turn-on' action for a socket device would call a method to activate that specific device using its deviceID.
